...
class var_HINITEModel(keras.Model):
    def __init__(self,config,activation=tf.nn.relu,init_adj=None):
        super(var_HINITEModel, self).__init__()
        ...
        self.Mixer = ourlayers.Mixer(config['mix_hidden_size'],activation = self.activation,use_bias=True)
        ...
        for i in range(len(self.init_adjs)):
            temp_gnn = []
            for j in range(config["GNN_hidden_layer"]-1):
                g = ourlayers.GATLayer(att_embedding_size = config['GNN_hidden_shape'][i], head_num=config['head_num_att'], 
                                     activation = self.activation, use_bias=False)
                temp_gnn.append(g)
            g = ourlayers.GATLayer(att_embedding_size = config['GNN_hidden_shape'][i], head_num=config['head_num_att'], 
                                     activation = self.activation, use_bias=False,reduction='mean')
            temp_gnn.append(g)
            self.gnn_layers.append(temp_gnn)
    ...
    def call(self, inputtensor,train_idx,training = False):
        ...
        for i in range(len(self.gnn_layers)):
            cur_emb = GNN
            for j in range(len(self.gnn_layers[i])):
                cur_emb = self.gnn_layers[i][j]([cur_emb,self.init_adjs[i]])
                #cur_emb = tf.nn.dropout(cur_emb,self.GNN_dropout)
            GNN_embs.append(tf.expand_dims(cur_emb,axis=1))

        cur_GNN = tf.concat(GNN_embs,axis = 1)
        GNN = self.Mixer(cur_GNN)
    ...
    def CV_y(self,inputtensor,all_y,train_idx,training = False):
        input_tensor = inputtensor[:,:-1]
        input_t = tf.constant(inputtensor[:,-1],shape = [input_tensor.shape[0],1])
        #bias_list = tf.ones([len(input_tensor),1])
        features = input_tensor
...
def pre_no_interf(self,inputtensor,test_idx,only_T = False,training = False):
        ...
        train_hidden = tf.gather(h_rep_norm,test_idx)
        train_GNN = tf.gather(GNN_norm,test_idx)
...
class HINITEModel(keras.Model):
    ...
    def pre_no_interf(self,inputtensor,test_idx,only_T = False,training = False):
        ...
        for i in range(len(self.out_T_layers)):
            outnn_C = self.out_C_layers[i](outnn_C)
        output_C = self.layer_6_C(outnn_C)
        self.no_inf_y.append(output_T)
        self.no_inf_y.append(output_C)

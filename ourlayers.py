...
class MGATLayer(keras.layers.Layer):
    ...
    def __init__(self, att_embedding_size=8, head_num=8, activation=tf.nn.relu,
                 reduction='concat', use_bias=True, **kwargs):
        if head_num <= 0:
...
class HXGATlayer(keras.layers.Layer):
    def __init__(self, att_embedding_size=8,hete_att_size=8,  activation=tf.nn.relu,
                 hete_num = 2,reduction='concat', divide = False,use_bias=False, **kwargs):
        if  hete_num <= 0:
            raise ValueError('head_num must be a int > 0')
        """
            Input:
        ...
        #self.att_embedding_size = att_embedding_size
        #self.hete_att_size = hete_att_size
        ...
        if self.divide:
            ...
            for i in range(hete_num):
                g = GATLayer(att_embedding_size = att_embedding_size, head_num=1, 
                                         activation = self.activation, use_bias=False,reduction='mean')
                self.gats.append(g)
        else:
            self.gats=MGATLayer(att_embedding_size = att_embedding_size, head_num=1, 
                                         activation = self.activation, use_bias=False,reduction='mean')
...
class Mixer(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.att_weight = self.add_weight(name='att_weight',shape=[self.hidden_size],
                dtype=tf.float32,initializer=tf.keras.initializers.glorot_uniform())
        
        if self.use_bias:
            self.bias_weight = self.add_weight(name='bias', shape=[self.hidden_size],
                                               dtype=tf.float32,
                                               initializer=keras.initializers.Zeros())
    def call(self,inputs):
        #print("inputs of mixer",inputs.shape)
